...
|----------|---------|---------|
| `ODATANO_ACCESS_KEY` | unset | The ODATANO ACCESS key `oda_…` from [api.odatano.dev](https://api.odatano.dev), sent as `Authorization: Bearer`. The same variable configures NIGHTGATE-MCP. Against a direct instance: an `odat_…` agent grant (`x-agent-token`) or any other bearer |
| `ODATANO_ACCESS_URL` | `https://api.odatano.dev` | The gateway, or a direct ODATANO host app (`http://localhost:4004` for `cds watch`) |
| `ODATANO_ACCESS_USER` / `ODATANO_ACCESS_PASSWORD` | unset | Basic auth against a direct instance (CAP mocked / dev auth, e.g. `alice`); not for agents |
...
The key is the whole configuration: the gateway is the default URL. A local `cds watch` instance instead: `ODATANO_ACCESS_URL=http://localhost:4004` and `ODATANO_ACCESS_USER=alice`.
