...
description: HTTP 402 payment gating for SAP CAP. OData endpoints open after on-chain Cardano settlement. Built on @odatano/core.
---

# x402 Example App

`@odatano/x402` is a **payment-gating library** for SAP CAP applications, built on `@odatano/core`. It implements the **Cardano-x402-v2** specification: every gated request returns `HTTP 402 Payment Required` until the caller proves on-chain settlement. Gate with ADA or any native Cardano token.
