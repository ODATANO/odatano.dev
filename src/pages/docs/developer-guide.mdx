...
description: "Architecture, internals, public API, testing."
---
...
Six CDS services, 40 entities (some shared via re-projection), 57 actions:

| Service | Path | Entities | Actions |
|---------|------|---------:|--------:|
| `CardanoODataService` | `/odata/v4/cardano-odata/` | 22 | 19 read |
| `CardanoTransactionService` | `/odata/v4/cardano-transaction/` | 8 | 12 unbound + 1 bound |
| `CardanoSignService` | `/odata/v4/cardano-sign/` | 5 | 9 |
| `CardanoIndexerService` (2.0) | `/odata/v4/cardano-indexer/` | 2 | 4 |
| `CardanoWorkerService` (2.0) | `/odata/v4/cardano-worker/` | 2 | 6 |
| `CardanoAgentService` (2.0) | `/odata/v4/cardano-agent/` | 1 | 6 |
...
For test contexts that skip CAP bootstrap, use `SKIP_AUTO_INIT=true` + `createTestContext(['koios'], 'buildooor')`.
