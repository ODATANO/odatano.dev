...
<img class="only-day" src="/assets/docs/build-sign-submit.svg" alt="Build → Sign → Submit transaction flow" />
<img class="only-night" src="/assets/docs/build-sign-submit-dark.svg" alt="Build → Sign → Submit transaction flow" /> \
...
All transactions are built by **Buildooor** (`@harmoniclabs/buildooor`), the sole builder since v1.8.0 (CSL was removed). \
The `txBuilders` config / `TX_BUILDERS` env var is still accepted for backward compatibility but is a no-op.
