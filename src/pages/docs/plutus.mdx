...
> **Buildooor is the sole transaction builder** (CSL was removed in v1.8.0). It computes the correct `scriptDataHash` for Plutus V3 mint and spend by construction — the old `PPViewHashesDontMatch` bug is resolved — and supports validity bounds, CIP-31 reference inputs, and `__INPUT_IDX__` placeholders. No builder configuration is needed.
...
Available on `BuildMintTransaction` and `BuildPlutusSpendTransaction`.
...
Buildooor converts via `posixToSlot()`. Defaults: `now − 120 s` / `now + 1 h`. Bounds must be numeric, non-negative, ≤ 13 digits, `start ≤ end`.
...
For validators that need the input index inside their redeemer, use the placeholder `__INPUT_IDX:<txHash>#<outputIndex>__` in `redeemerJson` / `mintRedeemerJson` / `inlineDatumJson`. Buildooor substitutes it after coin selection finalizes input ordering.

## Plutus Feature Support (Buildooor)

| Feature | Supported |
|---------|-----------|
| Plutus V3 spend / mint | ✅ |
...
| Combined spend+mint | ✅ |
| `extraOutputsJson` | ✅ |
| Plutus V1 / V2 (UPLC 1.0.0) | ❌ rejected at build time (V3 only) |
